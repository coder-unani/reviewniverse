...
export default function page() {
  ...
  const { mutateAsync: validateNickname, isPending: isValidatePending } =
    useValidateNickname();
  ...
  const EditSchema = Yup.object().shape({
    ...
    profile_text: Yup.string()
      .max(
        PROFILE_TEXT_MAX_LENGTH,
        `소개는 최대 ${PROFILE_TEXT_MAX_LENGTH}자까지 입력 가능합니다.`
      )
      .nullable(),
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    fieldsCheck.forEach((field) => {
      if (data[field] !== profile[field]) {
        if (field === 'profile_image' && data[field] === DEFAULT_IMAGES.noActor)
          return;
        if (field === 'profile_text' && !data[field] && profile[field] === null)
          return;
  ...
  useEffect(() => {
    if (watchIntroduction.length > PROFILE_TEXT_MAX_LENGTH) {
      setValue(
        'profile_text',
        watchIntroduction.slice(0, PROFILE_TEXT_MAX_LENGTH)
      );
    }
      ...
      <section className={styles.edit__section}>
        ...
        >
          ...
          <div className={`${styles.edit__input__wrapper} ${styles.nickname}`}>
            ...
            />
            {errors.nickname && (
              <p className={styles.edit__error}>{errors.nickname.message}</p>
            )}
          </div>
          <div
            className={`${styles.edit__input__wrapper} ${styles.introduction}`}
          >
          ...
          <div className={`${styles.edit__input__wrapper} ${styles.marketing}`}>
            <input
              type="checkbox"
              id="is_marketing_agree"
              {...register('is_marketing_agree')}
            />
          ...
          <button
            ...
            type="submit"
            disabled={
              !isDirty || !isValid || isValidatePending || isUpdatePending
            }
          >
            수정하기
          </button>
