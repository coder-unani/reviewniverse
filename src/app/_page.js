...
const VideosHorizontal = React.lazy(() => import('@/components/ui/VideosHorizontal'));
const VideosVertical = React.lazy(() => import('@/components/ui/VideosVertical'));
...
export default function page() {
  ...
  const renderScreenVideos = (code) => {
    ...
    // 코드에 해당하는 데이터만 필터링
    const screens = fExportScreenDataByCode(screenVideos.data, code);
    if (!screens) {
      return null;
    }
  ...
  return (
    <Suspense fallback={<SkeletonHome />}>
      <main className={styles.home__main}>
        <section className={styles.home__preview__section}>{renderScreenVideos('MA01')}</section>
