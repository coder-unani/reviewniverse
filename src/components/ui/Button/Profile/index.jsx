...
const ProfileButton = ({ user, size, onClose }) => {
  const profilePath = isEmpty(user) ? '' : EndpointManager.generateUrl(ENDPOINTS.USER, { userId: user.id });
  const profileImage = isEmpty(user) ? DEFAULT_IMAGES.noActor : user.profile_image;
  ...
  return (
    <Link href={profilePath} className={styles.profile} data-active={!isEmpty(user)} onClick={handleMobileMenuClose}>
