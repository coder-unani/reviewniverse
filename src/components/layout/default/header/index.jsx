  ...
  return (
    <section className={styles.header__wrapper}>
      ...
      <section className={styles.toolbar__container}>
        {isEmpty(user) ? <LoginLink /> : <ProfileLink user={user} />}
      </section>
    </section>
  );
};
...
// 모바일 헤더
const MobileHeader = ({ isSearch, handleMobileSearch, toggleMobileMenu }) => {
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (pathname !== ENDPOINTS.HOME && !isContentIdPath) return undefined;
  ...
  return (
    <header className={styles.header}>
      {isMobile ? (
        <MobileHeader isSearch={isSearch} handleMobileSearch={handleMobileSearch} toggleMobileMenu={toggleMobileMenu} />
      ) : (
